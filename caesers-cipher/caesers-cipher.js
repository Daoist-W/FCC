...
function ROT13b (str) {
  ...
  // creating a nested recursive function to retain information above
  function charConvert (arr) {
    let charCode = arr.join("").charCodeAt(0)
    if (65 <= charCode && charCode <= 77) {
       arrCont2.push(charCode + 13)
       arr.shift()
       charConvert(arr)
    }
    else if (77 < charCode && charCode <= 90) {
       arrCont2.push(charCode - 13)
       arr.shift()
       charConvert(arr)
    }
    else if (charCode) {
      arrCont2.push(charCode)
      arr.shift()
      charConvert(arr)
    }
}
